...
use WpifyMapyCzDeps\DI\Annotation\Inject;
use WpifyMapyCzDeps\DI\Annotation\Injectable;
use WpifyMapyCzDeps\DI\Definition\Exception\InvalidAnnotation;
use WpifyMapyCzDeps\DI\Definition\ObjectDefinition;
use WpifyMapyCzDeps\DI\Definition\ObjectDefinition\MethodInjection;
use WpifyMapyCzDeps\DI\Definition\ObjectDefinition\PropertyInjection;
use WpifyMapyCzDeps\DI\Definition\Reference;
use WpifyMapyCzDeps\Doctrine\Common\Annotations\AnnotationRegistry;
use WpifyMapyCzDeps\Doctrine\Common\Annotations\Reader;
use WpifyMapyCzDeps\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use WpifyMapyCzDeps\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'WpifyMapyCzDeps\\DI\\Annotation\\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'WpifyMapyCzDeps\\DI\\Annotation\\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('WpifyMapyCzDeps\\DI\\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'WpifyMapyCzDeps\\DI\\Annotation\\Injectable');
